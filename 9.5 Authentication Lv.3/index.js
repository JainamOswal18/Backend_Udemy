...
passport.use(
  ...
  new GoogleStrategy({
    ...
    try {
      ...
      const res = await db.query("Select * from auth where username = $1", [profile.email])
      if (res.rows.length === 0) {
        const newUser = await db.query("Insert into auth (username, password) values ($1, $2)", [profile.email, "google"])
