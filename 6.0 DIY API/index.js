...
app.get("/jokes/:id", (req, res) => {
  // var requestedJoke;
  // jokes.forEach(joke => {
  //   if (joke.id == req.params.id) requestedJoke = joke;
  // });
  var requestedJoke = jokes.find((joke) => joke.id == req.params.id );

  if (requestedJoke) {
    res.json(requestedJoke);
  } else {
    res.status(404).json("Invalid Joke ID!");
  }
});
...
app.get("/filter", (req, res) => {
  ...
  if (filteredJokes.length > 0) {
    ...
    res.status(404).json("No Joke Found");
  }
});
...
app.patch("/jokes/:id", (req, res) => {
  ...
  if (updatedJoke) {
    ...
    res.status(404).json("Not joke found!");
  }
});

//7. DELETE Specific joke
app.delete("/jokes/:id", (req, res) => {
  var jokeIndex = jokes.findIndex((joke) => joke.id == req.params.id);
